...
namespace proto_conv
{
    ...
    class conv
    {
        ...
        static void Main(string[] args)
        {
            if (args.Length < 1)
            {
                Console.WriteLine(@"Usage: proto_conv.exe <xlsx path>");
                return;
            }
            for (int i = 0; i < args.Length; i++)
            {
                string inputFilename = Path.GetFullPath(args[i]); //AppDomain.CurrentDomain.BaseDirectory + "test.xlsx";
                ...
                foreach (DataRow row in ttable.Rows)
                {
                    if (row["Table_Type"].ToString() == "TABLE")
                    {
                        ...
                        if (tabName.ToLower().Contains("description") || tabName.Contains("#_FilterDatabase"))
                            continue;
